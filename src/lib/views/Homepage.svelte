<script lang="ts">
	...
	function handleMouseWheel(event: Event): void{
		const e = event as WheelEvent;

		if(e.deltaY >= 4 || e.deltaY <= -4){
			slider.scrollTo({
				left: slider.scrollLeft + e.deltaY * 30,
				behavior: 'smooth',
			})
			e.preventDefault();
		}
...
{#if !isLoading}
	<div
		class="h-full flex flex-nowrap gap-2 no-scrollbar whitespace-nowrap overflow-x-scroll overflow-y-hidden"
	...
	>
			...
			{:else}
				<div
					class="flex justify-center items-center text-white text-5xl max-w-screen-md whitespace-normal mx-auto"
				>
					<h1>
						No games saved.<br />
						Add a new game by pressing
						<span>
							<svg
								on:click
								viewBox="0 0 24 24"
						...
						</span> on the top right to open settings
					</h1>
				</div>
			{/if}
	</div>
